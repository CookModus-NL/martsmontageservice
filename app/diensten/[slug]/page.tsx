...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!service) return {}
  return {
    title: `${service.title} in West-Brabant`,
    description: service.short,
    alternates: { canonical: `/diensten/${slug}` },
    openGraph: {
      title: `${service.title} | Marts Montage Service`,
      description: service.short,
      images: [service.hero],
    },
  }
          ...
          <div className="mt-12 grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
            <div className="reveal">
              ...
              <div className="mt-10 flex flex-wrap gap-3">
                <a
                  href={`https://wa.me/${business.whatsapp.replace('+', '')}?text=${encodeURIComponent('Hi Mart, ik heb een vraag over ' + service.title.toLowerCase() + '.')}`}
                  target="_blank"
      ...
      <section className="bg-primary-900 text-paper py-20">
        <div className="container-x">
          ...
          <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { title: 'Vaste vakman', body: 'Eén aanspreekpunt. Geen onderaannemers tenzij vooraf besproken.' },
              { title: 'Heldere offerte', body: 'Gespecificeerd, geen kleine lettertjes, binnen 5 werkdagen.' },
              { title: 'Schoon werk', body: 'Stof minimaal, puin direct afgevoerd, ruimte opgeruimd opgeleverd.' },
              { title: 'Garantie', body: '1 jaar op de werkzaamheden, fabrieksgarantie op materialen.' },
            ].map((item) => (
      ...
      <section className="bg-paper py-20 lg:py-24">
        <div className="container-tight rounded-3xl bg-primary-900 p-12 lg:p-16 text-center text-paper relative overflow-hidden">
          ...
          <p className="relative mt-4 text-primary-300 max-w-md mx-auto">
            Stuur foto’s en details via WhatsApp voor een indicatie binnen 1 werkdag.
          </p>
